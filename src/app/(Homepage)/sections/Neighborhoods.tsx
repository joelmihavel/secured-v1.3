...
const NEIGHBORHOOD_CONFIG: Record<string, { image: string; scheme: { bg: string; text: string; accent: string } }> = {
    "indiranagar": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 01_Indiranagar.png",
        scheme: { bg: "bg-forest-green", text: "text-night-violet", accent: "bg-pastel-violet" }
    },
    "mg road": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 01_MG Road.png",
        scheme: { bg: "bg-ground-brown", text: "text-brick-red", accent: "bg-pastel-pink" }
    },
    "bellandur": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 02_Bellandur.png",
        scheme: { bg: "bg-brick-red", text: "text-forest-green", accent: "bg-pastel-green" }
    },
    "haralur": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 02_Haralur.png",
        scheme: { bg: "bg-night-violet", text: "text-ground-brown", accent: "bg-pastel-orange" }
    },
    "hsr layout": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 03_HSR Layout.png",
        scheme: { bg: "bg-night-violet", text: "text-forest-green", accent: "bg-pastel-cyan" }
    },
    "whitefield": {
        image: "/Neighbourhoods/Neighbourhood Illustrations Set 03_Whitefield.png",
        scheme: { bg: "bg-forest-green", text: "text-night-violet", accent: "bg-pastel-yellow" }
    }
};
...
export const Neighborhoods = ({ locations, properties, children }: NeighborhoodsProps) => {
    ...
    const cards = neighborhoods.map((slide) => (
        <Link key={slide.id} href={`/homes?locationId=${slide.id}`} className="block h-full w-full" draggable={false}>
            <div className={`flex flex-col rounded-2xl ${slide.bgColor} overflow-hidden shadow-2xl w-full h-full`}>
                ...
                <div className={`relative flex-grow w-full overflow-hidden ${slide.bgColor} mx-auto h-[40vh]`} >
                    <Image
                        src={slide.image}
                        alt={slide.name}
                        fill
                ...
                {/* Bottom Label */}
                <div className={`py-1 md:py-2 ${slide.bgColor} text-white text-center font-heading tracking-wider text-xs md:text-sm z-20`}>
                    EXPLORE HOMES
                </div>
            </div>
        </Link >
    ...
    return (
        <OpenSection id="neighborhoods" className={`transition-colors duration-500 ease-in-out overflow-hidden`}>
            <div className={`absolute inset-0 opacity-10 bg-gradient-to-b from-white via-transparent to-white pointer-events-none`} />

            <div className="max-w-16xl relative z-10 py-12">
                <div className="text-center mb-8">
                    <h2 className={`font-heading text-text-main transition-colors duration-500 mb-4`}>
                        Explore by <span className="font-zin font-light"> <br /> Neighborhoods</span>
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 max-w-5xl mx-auto">
                    <div className="text-center">
                        <p className="text-4xl font-heading font-bold mb-2 text-text-main">150+</p>
                        <p className="text-sm text-text-main opacity-60">Homes Launched Across Bangalore</p>
                    </div>
                    <div className="text-center">
                        <p className="text-4xl font-heading font-bold mb-2 text-text-main">80%</p>
                        <p className="text-sm text-text-main opacity-60">Of our homes are in Tier I gated societies</p>
                    </div>
                    <div className="text-center">
                        <p className="text-4xl font-heading font-bold mb-2 text-text-main">4.8/5</p>
                        <p className="text-sm text-text-main opacity-60">Avg Rating From Flent Residents</p>
                    </div>
                </div>
            </div>
            {children}
