...
export interface StaticTweetData {
    user: {
        name: string;
        screen_name: string;
...
export const StaticTweetCard = ({
    ...
    tweet: StaticTweetData
    className?: string
}) => {
    return (
        <div
            className={cn(
                "relative flex h-fit w-full max-w-lg flex-col gap-2 overflow-hidden rounded-lg border p-4 backdrop-blur-md bg-white",
                className
            )}
            {...props}
        >
            ...
            {tweet.media && (
                <div className="flex flex-1 items-center justify-center mt-2">
                    {tweet.media.video ? (
                        ...
                        >
                            <source src={tweet.media.video} type="video/mp4" />
                            Your browser does not support the video tag.
                        </video>
                    ) : tweet.media.image ? (
                        <img
                            src={tweet.media.image}
