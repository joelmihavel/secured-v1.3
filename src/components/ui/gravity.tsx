...
type MatterBodyProps = {
    ...
    angle?: number
    className?: string
    delay?: number // Delay in milliseconds before adding to physics world
}
...
const MatterBody = ({
    ...
    useEffect(() => {
        ...
        // Delay registration if delay prop is specified
        const timer = setTimeout(() => {
            if (!elementRef.current || !context) return

            // Clear initial positioning styles before registering if they were applied
            if (delay > 0) {
    ...
    }, [props, children, matterBodyOptions, isDraggable, context, id, bodyType, sampleLength, x, y, angle, delay])
...
const Gravity = forwardRef<GravityRef, GravityProps>(
    (
        ...
        const updateElements = useCallback(function tick() {
        ...
        const initializeRenderer = useCallback(() => {
            ...
            // @ts-ignore - mousewheel is not in the type definition but exists at runtime
        ...
        return (
            <GravityContext.Provider value={{ registerElement, unregisterElement }}>
                <div
                    ref={canvas}
                    className={cn(className, "absolute inset-0 w-full h-full overflow-hidden")}
                    {...props}
                >
                    {children}
                </div>
