            ...
            <DrawerContent className="h-[90vh] md:h-[95vh] rounded-t-[20px] outline-none">
                ...
                <div className="h-full overflow-y-auto">
                    <div className="container mx-auto px-4 md:px-6 py-4 md:py-6 max-w-6xl">
                        {/* Header */}
                        <div className="flex items-center gap-1.5 md:gap-2 mb-6 md:mb-8 cursor-pointer w-fit" onClick={onClose}>
                            <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" />
                            <span className="text-xs md:text-sm font-medium"><span className="hidden sm:inline">Back to Apartment</span><span className="sm:hidden">Back</span></span>
                        </div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-8 md:gap-12 lg:gap-24">
                            ...
                            <div className="lg:w-1/2 space-y-6 md:space-y-8">
                                ...
                                <div>
                                    ...
                                    >
                                        <h3 className="text-base md:text-lg font-medium text-text-main">Rent Breakdown</h3>
                                        <div className="flex items-center gap-1 text-xs md:text-sm text-emerald-500 font-medium">
                                            ...
                                            {isBreakdownVisible ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                                        </div>
                                    </div>
                                ...
                                {/* One-Time Charges */}
                                <div>
                                    <h3 className="text-base md:text-lg font-medium text-text-main mb-3 md:mb-4">One-Time Charges</h3>
                                    <div className="flex justify-between text-xs md:text-sm text-text-main/80">
                                        <span>Society Move-In Fee</span>
                                        <span className="text-text-main/50">as per society norms</span>
                                    </div>
                                </div>
                            ...
                            <div className="lg:w-1/2 space-y-6 md:space-y-10">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                                    <div className="bg-pastel-brown/30 rounded-xl md:rounded-2xl p-4 md:p-6 text-center">
                                        <p className="text-xs md:text-sm font-medium text-text-main mb-3 md:mb-4">Monthly Rent</p>
                                        <div className="text-fluid-h3 font-heading text-text-main mb-1.5 md:mb-2">
                                            {formatCurrency(breakdown.totalRent)} <span className="text-sm md:text-base font-body text-text-main/70">/ mo</span>
                                        </div>
                                        <p className="text-[10px] md:text-xs text-text-main/60">GST Included</p>
                                    </div>
                                    <div className="bg-pastel-brown/30 rounded-xl md:rounded-2xl p-4 md:p-6 text-center">
                                        <p className="text-xs md:text-sm font-medium text-text-main mb-3 md:mb-4">Deposit Charges</p>
                                        <div className="text-fluid-h3 font-heading text-text-main mb-1.5 md:mb-2">
                                            {formatCurrency(breakdown.deposit)}
                                        </div>
                                        <p className="text-[10px] md:text-xs text-text-main/60">(Refundable)</p>
                                    </div>
                                </div>

                                {/* Add-Ons */}
                                <div>
                                    <h3 className="text-base md:text-lg font-medium text-text-main mb-1.5 md:mb-2">Add-Ons</h3>
                                    <p className="text-xs md:text-sm text-text-main/60 mb-4 md:mb-6">
                                        Prices listed are approximate per room. Flent doesn&apos;t manage these services.
                                    </p>
                                        ...
                                        {ADD_ONS.map((addon, index) => (
                                            <div
                                                ...
                                                className={cn(
                                                    ...
                                                    index % 4 === 0 ? "bg-gray-100 text-gray-700" :
                                                        index % 4 === 1 ? "bg-pastel-red/30 text-text-main" :
                                                            index % 4 === 2 ? "bg-pastel-green/30 text-text-main" :
                                                                "bg-pastel-violet/30 text-text-main"
                                                )}
                                            >
                                                {addon.name} <span className="opacity-60">+ {formatCurrency(addon.price)}{addon.unit}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
